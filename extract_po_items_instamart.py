...
def extract_po_header(text):
    ...
    # PO Date
    m = re.search(r'PO\s*Date\s*[:\-]\s*([A-Za-z]+\s+\d{1,2},?\s*\d{4}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4})', text, re.IGNORECASE)
    if m:
        fields["po_date"] = parse_date(m.group(1).strip())

    # Release Date
    m = re.search(r'(?:PO\s*)?Release\s*Date\s*[:\-]\s*([A-Za-z]+\s+\d{1,2},?\s*\d{4}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4})', text, re.IGNORECASE)
    if m:
        fields["release_date"] = parse_date(m.group(1).strip())

    # Expiry Date
    m = re.search(r'(?:PO\s*)?Expiry\s*Date\s*[:\-]\s*([A-Za-z]+\s+\d{1,2},?\s*\d{4}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4})', text, re.IGNORECASE)
    if m:
        fields["expiry_date"] = parse_date(m.group(1).strip())

    # Factory / Vendor Name — first non-empty line after "Vendor Name"
    m = re.search(r'Vendor\s*Name\s*[:\-]?\s*\n([^\n]+)', text, re.IGNORECASE)
    if m:
        name = m.group(1).strip()
        # Remove PO No part if on same line
        name = re.sub(r'PO\s*No.*', '', name, flags=re.IGNORECASE).strip()
        if name:
            fields["factory_name"] = name

    # Fallback: grab vendor name from the block before "PO Date"
        ...
        for i, line in enumerate(lines):
            if re.search(r'Vendor\s*Name', line, re.IGNORECASE):
                ...
                for j in range(i+1, min(i+5, len(lines))):
                    candidate = re.sub(r'PO\s*(No|Date|Release|Expiry).*', '', lines[j], flags=re.IGNORECASE).strip()
        ...
        for page_num, page in enumerate(pdf.pages, 1):
            ...
            for table in tables:
                ...
                for i, row in enumerate(table):
                    if row is None:
                        continue
                    row_text = [clean_text(c).lower() for c in row]

                    has_item_code = any('item' in c and 'code' in c for c in row_text)
                    has_qty = any(c in ('qty', 'quantity') for c in row_text)
                    has_desc = any('desc' in c or 'description' in c for c in row_text)
                    ...
                    if has_item_code or (has_qty and has_desc):
                        ...
                        for j, cell in enumerate(row_text):
                            if 'item' in cell and 'code' in cell:
                                col_map['item_code'] = j
                            elif 'item' in cell and ('desc' in cell or 'name' in cell):
                                col_map['item_desc'] = j
                ...
                if header_row_idx is None:
                    for i, row in enumerate(table):
                        ...
                        if 'item code' in combined or ('qty' in combined and ('desc' in combined or 'item' in combined)):
                            header_row_idx = i
                            for j, cell in enumerate(row_text):
                                if 'item code' in cell:
                                    col_map['item_code'] = j
                                if 'item desc' in cell or 'description' in cell:
                                    col_map['item_desc'] = j
                ...
                for row in table[header_row_idx + 1:]:
                    ...
                    if any(kw in combined for kw in ['total', 'grand total', 'amount in words', 'subtotal']):
                        continue
                    ...
                    item_code = clean_text(row[col_map['item_code']]) if 'item_code' in col_map else ''
                    item_desc = clean_text(row[col_map['item_desc']]) if 'item_desc' in col_map else ''
                    qty       = clean_text(row[col_map['qty']])       if 'qty'       in col_map else ''

                    item_desc = re.sub(r'(Colour|Color|Size|Brand)\s*:.*', '', item_desc, flags=re.IGNORECASE).strip()
                    item_desc = re.sub(r'\s+', ' ', item_desc).strip()
                    item_desc = re.sub(r'(?<=[A-Za-z]) (?=[a-z])', '', item_desc)
...
if __name__ == "__main__":
    ...
    pdf_path = args[0] if args else r"C:\Users\admin\Downloads\SOTY-1N99738266-JC2PO02359.pdf"
